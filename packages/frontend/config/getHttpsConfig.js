...
function validateKeyAndCerts({ cert, key, keyFile, crtFile }) {
  ...
  try {
    ...
    throw new Error(`The certificate "${yellow(crtFile)}" is invalid.\n${err.message}`);
  }
  ...
  try {
    ...
    throw new Error(`The certificate key "${yellow(keyFile)}" is invalid.\n${err.message}`);
  }
}
...
function readEnvFile(file, type) {
  if (!existsSync(file)) {
    throw new Error(`You specified ${cyan(type)} in your env, but the file "${yellow(file)}" can't be found.`);
  }
